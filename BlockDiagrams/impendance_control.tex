...
\node [block, above = 0.5 of sum2] (jt) {$ \textbf{J}^T $};
\node [support, above = 0.5 of jt](supjt) {}; 
...
\node [output, left = +0.7 of output] (outputq) {};
\node [output, above left = +0.7 of output] (outputhe) {};
%\node [output, below right = +0.7 of outputdq] (outputddq) {};



\draw [->] (inputdq) node[above = 0.1 cm of inputdq]{$\mathbf{\dot{x}}_{ref}$} -- node[pos=0.9] {{\tiny $+$}} (sumdq);
\draw [->] (sumdq) -- (kd);
...
\draw[->] ($(pa10.0) + (0,1/2)$) -- node[above =-0.1 cm, pos=0.5]{$\textbf{h}_e$} ($(output.0) + (1.5,1/2)$);
\draw[->] ($(pa10.0) + (0,0)$) -- node[above =-0.1 cm, pos=0.1]{$\textbf{q}$} ($(output.0) + (1.5,0)$) ;
\draw[->] ($(pa10.0) + (0,-1/2)$) -- node[above =-0.1 cm, pos=0.5]{$\dot{\mathbf{q}}$} ($(output.0) + (1.5,-1/2)$) ;



%\draw [->] (outputq) -- +(0,-4) -| node[pos=0.99, left]{\tiny {$-$}} (sumq);
\draw [->] (outputhe) |- ($(supjt.0)+(-6,0)$) -- node[pos=0.99, right]{\tiny {$+$}}(sumall);
\draw [->] (supjt)  -- (jt) -- node[pos=0.99, left]{\tiny {$+$}}(sum2);
%\draw [->] ($(sumall) + (1/2,1.3)$) -- (sumall);
\draw [->] (outputq) |- (gmod);
...
\fill (outputhe)circle [radius=2pt];


\fill ($(outputdq.0) + (0,-1.65)$)circle [radius=2pt];
\fill ($(outputq.0) + (0,-1.75)$)circle [radius=2pt];

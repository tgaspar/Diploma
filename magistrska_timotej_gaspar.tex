...
Podjetje Mitsubishi Heavy Industries je leta 1992 izdelalo prvega industrijskega redundatnega robota \cite{mhi_pa10}. Podjetje je robota poimenovalo Portable General-purpose Intelligent Arm PA-10, krajše PA-10. Gre za serijskega robota s sedmimi stopnjami prostosti. Prvi trije sklepi so oznaèeni kot ramenski sklepi (shoulder), S1, S2, S3. Naslednja dva sta oznaèena kot komolèni sklepi (elbow), E1, E2. Zadnja dva sklepa pa sta oznaèena kot zapestna (wrist), W1, W2. Glede na zgradbo se ga lahko uvrsti v tako imenovane antropomorfne robote \cite{craig_robintro}. Izraz antropomorfnost izhaja iz dveh grških besed \textit{antropo} - èlovek, \textit{morphe} - oblika. Znaèilnost takih robotov je spretnost saj so vsi sklepi rotacijski \cite{siciliano_robot}.
...
\section{Kinematièni model robota PA-10}

Robotski mehanizem se obravnava kot kinematièno verigo \textit{n} med seboj povezanih togih teles. Ker je en konec kinematiène verige povezan v bazo robota je mogoèe reèi, da se premika le vrh robota. Z opisom kinematiène relacije med dvema zaporednima segmentoma je mogoèe to znanje združiti za definiranje kinematiène relacije med bazo in vrhom robota. Homogena transformacijska matrika med dvema sosednjima segmentoma je definirana kot:
...
V homogeni transformacijski matriki $\textbf{T}_{i+1}$ so $\textit{a}_i$, $\alpha_i$, $d_i$ in $\varTheta$ parametri tako imenovane Denavit - Hartenberg sistema opisovanja relacij med razliènimi koordinatnimi sistemi. Sklicujoè na sliko \ref{fig:dh-pa10} se opiše parametre kot:
...
Zgornje parametre se d

Tako je mogoèe zapisati homogeno transformacijo od baze do vrha robota kot produkt posameznih transformacij

\begin{equation} \label{eq:tob}
	T_{7}^0 = T_1^0 \cdot T_2^1 \cdot ... \cdot T_7^6.
\end{equation}

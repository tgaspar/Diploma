...
%*************************** NASLOVNA STRAN *****************************
\naslov{Implementacija hitrostnega ter navornega krmilnjenja na industrijskem robotu Mitsubishi PA-10 z uporabo namensko zgrajenega UDP-ARCNET strežnika}
...
Pri tem delu avtorji predstavijo namensko narejen strežnik, ki deluje kot posrednik med UDP ter ARCNET komunikacijo s krmilnikom servo motorjev robota PA-10. Strežnik je nastal z zahtevami po delovanju v realnem èasu in frekvenci cikla 500 Hz.

Raziskalo se je strategije vodenja robotov in se jih je implementiralo na robotu PA-10. Napravilo se je poizkuse s hitrostnim ter navornim naèinom vodenja motorjev v zunanjih in notranjih koordinatah. Dodatno se je še ugotovilo možnosti krmiljenja robota v kontaktu z okolico. Poizkusi so pokazali, da je hitrostno krmiljenje lažje za implementirati, saj ni tako odvisno od dinamiènih parametrov robota. Navorno vodenje se je v tem primeru izkazalo za nezadovoljivo ampak to predvsem zaradi nenatanènega dinamiènega modela.

Dodatno je so med tem delom nastala orodja za krmiljenje robota s programskim paketom \matsim. Na ta naèin se omogoèa nadaljnje delo na robotu za razvijanje algoritmov vodenja.
...
For this thesis the authors have developed a server that acts like a man in the middle for a UDP client and ARCNET servo controller of the PA-10 robot. The server was developed with the requirement to work as close to real time as possible with a program cycle of 500 Hz.

Robot control strategies were reviewd and implemented on the PA-10. Experiments were made for velocity and torque mode control in joint and task space. Robot control in contact with enviroment was also tested. Through experiments we found that velocity control of robot is easier to implement as it is not so dependent on the dinamic parameters of the robot. Torque control was found to be unsatisfactory due to the model being innacurate.

In addition \matsim libraries were developted for further robot use and research.
...
V nadaljevanju tega poglavja, bo opisana implementacija algoritmov vodenja, ki so bili opisani v poglavju \ref{chap:vodenje}. Implementacija vodenj je hkrati tudi preizkus s katerim se bo ovrednotilo namensko narejen strežnik.

Vodenje bo je bilo realizirano z uporabo strežnika opisanega v poglavju \ref{chap:udp-arcnet-server}. Za komunikacijo s strežnikom je bilo narejeno visoko nivojsko vodenje v programskem paketu \matsim. Razlogi za izbiro omenjenega programskega paketa so bile dobre predhodne izkušnje ter dobro poznavanje delovanja programa s strani avtorjev. \simulink \space bloki, ki smo jih uporabili, bodo v nadaljevanju prikazani na slikah in podrobneje opisani v dodatku \ref{appendix:simulink-block}.

Poizkusi za hitrostno in navorno vodenje so potekali z enakimi referenènimi signali, da se lahko med sabo primerja rezultate. Za oba naèina vodenja smo opravili tri poizkuse z razliènimi referenènimi signali:
\begin{enumerate}
	\item signal stopnice v notranjih koordinatah,
	\item sinusni signal z narašèajoèo frekvenco v notranjih koordinatah,
	\item zarisovanje kroga v zunanjih koordinatah.

\end{enumerate}

Stopnièasto referenco smo izbrali, ker je odziv nanjo dober pokazatelj kako se sistem odziva na hitro spremembo. Preden smo nastavili ojaèanja smo zadali pogoj, da odziv ne sme imeti prenihaja oz. naj bo le-ta najmanjši možen. Odziv smo ovrednotili na podlagi t.i. inženirskih pokazateljev \cite{zupancic_vs}, ki opisujejo uèinkovitost delovanja regulacijskega sistema:
\begin{itemize}
	\item mrtvi èas ($ t_m $), sistem še ne odzove na krmiljenje,	
	\item èas vzpona ($ t_r $), regulirana velièina naraste iz 10\% do 90\% konène vrednosti,
	\item èas maksimalnega prevzpona ($ t_p $), odziv doseže maksimalno vrednost,
	\item maksimalni prevzpon ($ M_p $),
	\item umiritveni èas ($ t_s $), odziv doseže in ostane znotraj 2\% referenène vrednosti.
\end{itemize}

Sinusni signal z narašèajoèo frekvenco smo izbrali zato, ker nam pokaže kako je napaka odvisna od hitrosti spreminjanja reference. Predpostavka je bila, da višja kot je frekvenca, veèja je napaka. 

Zarisovanje kroga z vrhom robota smo izbrali zato, da vodimo vrh robota po prostoru naloge in vidimo, kako dobro deluje sledenje v zunanjih koordinatah. Predpostavka je bila, da z dobrim regulatorjem lahko dosežemo dobro sledenje spreminjajoèi se referenci.

Ovrednotili smo tudi vodenje robota v kontaktu z okolico tako pri hitrostnem (admitanènem) kot tudi pri navornem (impedanènem) vodenju. V kontaktu z okolico je bil referenèni signal oblikovan tako, daje robot deloval z želeno silo na podlago. Preizkuse smo opravili na referencah dveh razliènih oblik in sicer stopnièaste in sinusne. Za obe razlièni referenci smo opravili tri razliène poizkuse. Pri stopnièastem signalu smo spreminjali amplitudo, pri sinusnem pa frekvenco.

\section{Hitrostno krmiljenje}



Implementacija hitrostnega krmiljenja robota je potekala postopoma. Najprej smo realizirali pozicijsko vodenje sklepov nato pa vodenje v zunanjih koordinatah.

\input{./Slike/follow_step.tex}

Odziv tretjega sklepa na referenèni signal stopnice je viden na sliki \ref{fig:follow_step}. Iz pridobljenih podatkov podatkov lahko izmerimo prej definirane inženirske pokazatelje.
\input{./Tabele/vel_step.tex}
Na grafu je videti, da krmilni signal doseže referenèno vrednost relativno hitro nato pa se za kratek èas od nje spet oddali. To je posledica dinamiènih lastnosti robota in PI regulatorja. Ko se sklep približa referenèni legi zaène motor zavirati in zaradi vztrajnosti se poveèuje I èlen regulatorja. Hitrost pade na vrednost niè, vendar pa je I èlen regulatorja še vedno visok in posledièno se sklep malo zavrti stran od reference. Pomembno je opozoriti tudi na mrtvi èas, ki je 0.008 s. To pomeni, da je v komunikaciji med visokonivojskim in nizkonivojskim krmilnikom zamika za 4 vzorce. V tem naèinu vodenja to sicer ne predstavlja težav, jih pa lahko predstavlja pri vodenju s silo.

\input{./Slike/follow_sine.tex}

Graf na sliki \ref{fig:follow_sine} prikazuje odziv tretjega sklepa na referenèni signal sinusne oblike. Referenènemu signalu je frekvenca narašèala od 0.1 Hz do 1 Hz v èasovnem intervalu 45 sekund. Na ta naèin se je preverilo, kako dobro še sledi referenci, ko se ta hitreje spreminja. Z rdeèo obarvan signal napake prikazuje, da je sledenje s poveèevanjem hitrosti slabše.

\input{./Slike/follow_circle.tex}
Tretji poizkus vodenja robota je temeljil na vodenju v zunanjih koordinatah. Referenèna trajektorija je bila v obliki kroga na $x$ in $ y $ ravnini. Graf je na sliki \ref{fig:follow_circle}. Na grafu je videti, da vrh robota dobro sledi referenci. Graf \ref{fig:follow_circle_err} prikazuje potek signala napake. Najveèja napaka v sledenju je bila 0.006 m.

\input{./Slike/follow_circle_err.tex}
...
Admitanèno vodenje smo implementirali ter preverili delovanje preko sledenja referenèni sili. Robot (slika \ref{fig:pa10jr3}) je pritiskal ob leseno površino, ki je bila zaradi varnosti postavljena na tršo spužvo. Na ta naèin smo dosegli to, da je robot pritiskal na relativno tog objekt, vendar pa bi v primeru premoènega potiska spužva ublažila udarce. 

Sledenje stopnici prikazujejo grafi na sliki \ref{fig:admitance_step}. Vidimo lahko, da amplituda prenihaja ostaja v podobnem sorazmerju z referenco in se giblje okoli 50\% reference. Velik prenihaj je posledica uporabe integratorja. Se pa z veèanjem amplitude reference zmanjšuje umiritveni èas. To pa zato, ker so oscilacije v ustaljenem stanju približno enake amplitude in posledièno pri veèjih referencah predstavljajo manjši odstotek.

\input{./Slike/admitance_step.tex}

Sledenje sinusni referenci prikazuje slika \ref{fig:admitance_sine}. Na grafih je opaziti, da s poveèevanjem frekvence se poveèuje tudi napaka. Ta pojav smo opazili tudi pri hitrostnem vodenju in je prièakovano, da je tudi tukaj prisoten.

\input{./Slike/admitance_sine.tex}

\section{Navorno krmiljenje}

Za implementacijo navornega vodenja je bilo najprej potrebno definirati dinamiène parametre. Robot PA-10 je precej razširjen in so nekateri raziskovalci te parametre že identificirali. Podatke o masi robota smo povzeli iz \cite{aalst_pa10,voung_pa10,rodrigo_pa10,petric_nevronska}. Parametre za model trenja smo identificirali sami. 
...
Parametre $ f_1 \dots f_6 $ smo identificirali z eksperimentom. Ker krmilnik robota v vsakem trenutku vraèa navor, ki ga daje na motorje, smo robota vodili hitrostno in opazovali, kakšen navor proizvaja v sklepih. Pri tem eksperimentu smo morali robota postaviti v tako konfiguracijo, da gravitacija ni imela vpliv na navor v sklepih. Slika \ref{fig:friction_measure} prikazuje navore v odvisnosti od hitrosti. Iz pridobljenih podatkov smo s pomoèjo \matlab   funkcije za optimizacijo identificirali parametre Stribeckovega modela za vsak sklep razen za drugega. Ker je bila baza robota pritrjena na tla nismo morali postaviti robota v tako konfiguracijo, da gravitacija ne bi imela vpliva na navor proizveden v drugem sklepu, kar je tudi vidno na grafu \ref{fig:friction_measure_2}. Za ta sklep smo parametre doloèili arbitrarno. Identificirani parametri so zapisani v tabeli \ref{table:friction_params}.
\input{./Tabele/friction_params.tex}


\subsection{Sledenje referenci}

Za izvedbo navornega vodenja robota smo uporabili dinamièni model, ki ga je zgradil \cite{petric_nevronska}. Dodatno smo uporabili identificirane parametre trenja za kompenzacijo trenja. Izvedba vodenja je potekala postopoma. Najprej se je preverilo delovanje modela tako, da se je robota vodilo le s kompenzacijo gravitacije. Predpostavka je bila, da v kolikor je vpliv gravitacije dobro kompenziran, bo robot ohranjal konfiguracijo v katerikoli legi. Ko smo to dosegli, so sledili poizkusi kot pri hitrostnem vodenju.

\input{./Slike/torque_follow_step.tex}

Slika \ref{fig:torque_follow_step} prikazuje graf odziva tretjega sklepa na stopnièasto referenco. Z razliko od hitrostnega vodenja v tem primeru regulirana velièina ni dosegla referenène vrednosti. Razlog za tem je ne tako dobro kompenzirano trenje. Krmilnik sicer daje nek navor na sklep, vendar je ta premajhen da bi premagal silo trenja. Inženirski pokazatelji vodenja za odziv na stopnico so zapisani v tabeli \ref{table:tq_step}.
\input{./Tabele/tq_step.tex}


\input{./Slike/torque_follow_sine.tex}

Graf na sliki \ref{fig:follow_sine} prikazuje odziv tretjega sklepa na referenèni signal sinusne oblike. Referenèni signal je bil enak kot pri hitrostnem vodenju. Èe primerjamo ta graf z grafom hitrostnega vodenja opazimo, da tukaj napaka s višjo frekvenco ne narašèa ampak le spreminja obliko. Razlog za tem je uporaba prvega in drugega odvoda reference za tvorjenje krmilnega signala.


\input{./Slike/torque_follow_circle.tex}
Navorno vodenje robota po trajektoriji v zunanjih koordinatah po krožnici ni bilo tako uspešno kakor pri hitrostnem vodenju. Napaka je pri sledenju bila mnogo veèja kot pri hitrostnem vodenju. Slika \ref{fig:torque_follow_circle} prikazuje graf sledenja, slika \ref{fig:torque_follow_circle_err} pa napako sledenja. Videti je, da sledenje ni tako uspešno. Iz grafa lahko vidimo, da je najveèja napaka malo veè kot 6 mm
\input{./Slike/torque_follow_circle_err.tex}

Problematika navornega krmiljenja je, da je pogojena z dobrim poznavanjem dinamiènega modela. Vsako odstopanje modela od fiziènega robota se pokaže kot napaka v sledenju. Za boljše sledenje bi bilo potrebno dinamiène parametre boljše identificirati.
...
%BEGIN_FOLD
%Za visoko nivojsko vodenje smo uporabljali programski paket \matsim. Ker je želja avtorjev, da se raziskovalno in razvojno delo na robotu PA-10 v prihodnosti nadaljuje, so nastale \simulink knjižnice. Knjižnice bodoèim uporabnikom omogoèajo uporabo robota brez, da poznajo do podrobnosti vse, kar je bilo opisano do sedaj v tem delu. V nadaljevanju bodo opisani posamezni bloki, ki jih lahko uporabniki dodajo v njihovo \simulink shemo.
%
%\section{Blok PA-10}
%
...
Skozi delo smo avtorji predstavili dve strategiji vodenja robota ter opisali reševanje problema redundantnosti. Predstavili smo na novo narejen krmilnik, ki deluje kot strežnik ter ovrednotili njegovo delovanje. Skozi eksperimente smo pokazali zadovoljivo delovanje strežnika. Pri eksperimentih smo se tudi seznanili s težavo uporabljenega dinamiènega modela robota PA-10

Opazili smo, da vodenje robota preko vhodnih hitrostih dela prièakovano in zadovoljivo. Vsi eksperimenti so pokazali dobre odzive in majhne napake. Tudi admitanèno vodenje preko referenène sile se je dobro izkazalo z hitrim odzivom in umiritvenim èasom. Prenihaj bi lahko zmanjšali vendar bi s tem tudi zmanjšali dinamiko odziva.

Navorno vodenje robota se je z danim dinamiènim modelom izkazalo kot nezadovoljivo. Odziv na stopnico je bil sicer hitrejši vendar pa napaka ni konvergirala proti niè. Sledenje referenèni trajektoriji v zunanjih koordinatah je imelo veèjo napako kot pri hitrostnem vodenju predvsem pa je bila napaka neenakomerna.

Vse eksperimente smo izvedeli s krmilniki, ki smo jih avtorji izdelali. Nastala so orodja, ki bodo lahko še naprej uporabljena za nadaljnje delo z robotom PA-10.

Nadaljnje delo predvideva izboljšanje identificiranih parametrov dinamiènega modela za boljše vodenje v navornem naèinu. Predvsem se bo izboljšalo parametre za kompenzacijo trenja tako, da dobro delovalo tudi pri nizkih hitrostih. Na ta naèin se bo doseglo dobro konvergenco napake proti niè.
...
Simulink knjižnica, ki je nastala za uporabljanje robota PA-10 v \matsim je prikazana na sliki \ref{fig:simulink-pa-10}. Blok ima 5 vhodov in 5 izhodov. Vhodi v blok so poleg krmilnih š

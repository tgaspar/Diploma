...
\dot{\textbf{x}} = \textbf{J}(q) \cdot \dot{\textbf{q}}
\end{equation}
...
Z obratom enaèbe \ref{qtox} se izpostavi $\tilde{q}$:

\begin{equation} \label{qerr}
\tilde{\textbf{q}} = \textbf{J}^{-1}(q) \cdot \tilde{\textbf{x}}
\end{equation}
...
\textbf{u} = \textbf{K}_p \cdot \textbf{J}^{-1}(q) \cdot (\textbf{x}_{ref} - \textbf{x})
\end{equation}
...
Napako sile prištejemo referenèni pozicijo po tistem, ko jo ojaèamo z matriko ojaèanj $\textbf{K}_h$. Matrika je diagonalna $K_h$ in pove, koliko je vrh robota podajen v posamezne smeri. Novo referenèno pozicijo se bo oznaèilo z $\textbf{x}^*_{ref}$.

\begin{equation} \label{newx}
	\textbf{x}^*_{ref} = \textbf{x}_{ref} + \tilde{\textbf{h}}
\end{equation} 
...
\textbf{u} = \textbf{K}_p \cdot \textbf{J}^{-1}(q) \cdot (\textbf{x}_{ref} + \tilde{\textbf{h}} - \textbf{x})
\end{equation}

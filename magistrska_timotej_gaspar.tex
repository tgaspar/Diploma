...
V nadaljevanju tega poglavja, bo opisana implementacija algoritmov vodenja, ki so bili opisani v poglavju \ref{chap:vodenje}. Implementacija vodenj je hkrati tudi preizkus s katerim se bo ovrednotilo razviti UDP-ARNCET strežnik, ki je bil opisan v poglavju \ref{chap:udp-arcnet-server}. Za komunikacijo s strežnikom je bilo narejeno visoko nivojsko vodenje v programskem paketu \matsim. Razlogi za izbiro omenjenega programskega paketa so bile dobre predhodne izkušnje ter dobro poznavanje delovanja programa s strani avtorjev. \simulink \space bloki, ki smo jih uporabili, bodo v nadaljevanju prikazani na slikah in podrobneje opisani v dodatku \ref{appendix:simulink-block}.

Namen poskusov je bil ovrednotiti razviti strežnik. Pokazati smo želeli, kako je višanje frekvence delovanja vplivalo na uspešnost vodenja. V ta namen, samo zgradili testni strežnik, ki deluje s tako frekvenco kot je deloval MHI krmilnik, in sicer 100 Hz. Z obema strežnikoma smo izvedeli enake poskuse vodenja robota in sicer:

\begin{enumerate}
	\item odziv na stopnièasti referenèni signal pozicije v notranjih koordinatah,
	\item odziv na referenèni signal pozicije sinusne oblike v zunanjih koordinatah,
	\item odziv na stopnièasti referenèni signal sile v zunanjih koordinatah.
\end{enumerate}
Vsi poskusi so bili izvedeni s hitrostnim in na navornim vodenjem.
	
Stopnièasti signal je bil izbran zato, da smo ugotovili, kako visoko lahko nastavimo ojaèanje P regulatorja, da regulirani signal nima prenihaja. Dodatno smo tukaj lahko tudi kakšna je zakasnitev v komunikaciji. Z veèjimi vzorènimi èasi se lahko doseže veèja ojaèanja in posledièno tudi hitrejše odzive. 
...
\begin{equation}
x_{ref} = - 0,01\sin(f(t) 2 \pi t); \quad \begin{cases}
f(t=0) = 0.1\\
f(t=45) = 3
\end{cases}.
\end{equation}
...
Ovrednotili smo tudi vodenje robota v kontaktu z okolico tako pri hitrostnem (admitanènem) kot tudi pri navornem (impedanènem) vodenju. V kontaktu z okolico je bil referenèni signal oblikovan tako, da je robot deloval z želeno silo na podlago. Poskus je bil opravljen na stopnièastem signalu na podlagi z veliko togostjo. 
...
Za pozicijsko vodenje sklepov smo uporabili regulator opisan na shemi \ref{fig:qfeedback}. Grafi odzivov so prikazani na sliki \ref{fig:follow_step}. Iz grafov lahko opazimo, da z vzorèno frekvenco 500 Hz lahko uporabimo veèja ojaèanja za doseganje hitrejših odzivov. Pri ojaèanju $ K_p = 13 $ doseže sklep 98\% referenène vrednosti v 0.40 sekunde, z vzorèno frekvenco pa je $ K_p = 10 $ in doseže 98\% referenène vrednosti v 0.47 sekunde. 

Za vodenje robota v zunanjih koordinatah po izbrani trajektoriji smo uporabili regulacijski zakon zapisan v \ref{eq:outcontrol}. Graf \ref{fig:follow_sine} prikazuje sledenje referenci.

\input{./Slike/follow_sine.tex}

Graf na sliki \ref{fig:follow_sine} prikazuje odziv tretjega sklepa na referenèni signal sinusne oblike. Referenènemu signalu je frekvenca narašèala od 0.1 Hz do 3 Hz v èasovnem intervalu 45 sekund. Na ta naèin se je preverilo, kako dobro še sledi referenci, ko se ta hitreje spreminja. Ojaèanja so bila empirièno nastavljena do meje stabilnosti. Z rdeèo obarvan signal napake nam prikazuje, da je sledenje s poveèevanjem hitrosti v obeh primerih slabše, vendar pa pri vzorènem èasu 100 Hz amplituda napake hitreje narašèa. Napako bi lahko odpravili z dodatkov v naprej zakljuèene zanke odvoda referenènega signala

\begin{equation} \label{eq:outcontrol_ff}
	\textbf{u} =  \textbf{J}^{*}(\textbf{q})(\textbf{K}_p\mathbf{\tilde{x}}_{ref} + \mathbf{\dot{x}}_ref).
\end{equation}
...
Admitanèno vodenje smo implementirali ter preverili delovanje preko sledenja referenèni sili. Za vodenje robota smo uporabili regulacijski zakon \ref{eq:admitcontrol}. Robot (slika \ref{fig:pa10jr3}) je pritiskal na leseno površino, ki je bila zaradi varnosti postavljena na tršo spužvo. Na ta naèin smo dosegli to, da je robot pritiskal na relativno objekt z relativno trdo površino.

Rezultati poskusa za sledenje stopnièasti referenci so prikazani na sliki \ref{fig:admitance_step}. Najprej doloèili ojaèanja regulatorja na meji stabilnosti za vodenje z vzorèno frekvenco 500 Hz ter opravili poskus. Poskus smo z enakimi ojaèanji ponovili z vzorèno frekvenco 100 Hz. Na grafih je videti, da je je vodenje pri frekvenci 500 Hz še stabilno pri vodenju s 100 Hz pa veè ne. Opaziumo tudi veèje intervale mirovanje, ko se je robot oddalil od površine. To je posledica uporabe integratorja v krmilni enaèbi. Integrator integrira napako ter poveèuje krmilni signal, da doseže referenèno silo. Ko jo doseže zaène to nemudoma zmanjševati. Ker je vzorèni èas prevelik to vodi v nestabilnost. Pri 500 Hz ni teh težav, kar tudi potrjuje navedbe \cite{mihelj_hapt}, da je za vodenje robot v kontaktu z okolico potrebno imeti visoke vzoršène frekvence.
...
Za implementacijo navornega vodenja je bilo najprej potrebno definirati dinamiène parametre. Robot PA-10 je precej razširjen in so nekateri raziskovalci te parametre že identificirali \cite{voung_pa10,aalst_pa10,rodrigo_pa10,petric_nevronska}. Parametre za model trenja smo identificirali sami, ker so se lahko z obrabljenostjo, gretjem ter èasom spreminjajo.

Dinamièni model, ki smo ga za poskuse uporabljali je zgradil \cite{petric_nevronska} z uporabo orodja \textit{sdfast}. Avtor je za dinamièni model uporabil podatke o masah zapisani v tabeli \ref{table:grav_params}. Matrike vztrajnosti za vsak sklep je avtor aproksimiral z matriki dimenzije $ 3 \times 3$, ki ima le diagonalne èlene razliène od niè. Za kompenzacijo trenja smo uporabili parametre, ki smo jih identificirali sami. 
...
Izvedba vodenja je potekala postopoma. Tako kot pri hitrostnem vodenju  smo najprej opravili poskus s stopnièasto referenco v notranjih koordinatah nato pa v odziv na sinusno funkcijo v zunanjih koordinatah. 

\input{./Slike/torque_follow_step.tex}

Za izvedbo poskusa smo uporabili regulacijski zakon \ref{eq:tq_in_coor}. Slika \ref{fig:torque_follow_step} prikazuje graf odziva tretjega sklepa na stopnièasto referenco. Podobno kot pri hitrostnem smo z višanjem vzorène frekvence lahko uporabili tudi veèja ojaèanja. Vendar pa je bila v tem primeru razlika v ojaèanjih veèja. S frekvenco vzorèenja 100 Hz smo z ojaèanjem 10 dosegli 98\% referenène vrednosti v 0.24 sekunde. S frekvenco vzorèenja 500 Hz pa je bilo ojaèanje 21 in èas vzpona 0.184 sekunde.
...
Graf na sliki \ref{fig:torque_follow_sine} prikazuje odziv vrha robota na referenèni signal sinusne oblike. Referenèni signal je bil enak kot pri hitrostnem vodenju. Ojaèanja vodenja so bila nastavljena do meje stabilnosti. Opaziti je ponovno boljše sledenje referenci pri veèji vzorèni frekvenci. Vendar pa je potrebno opozoriti na splošno slabše sledenje kot pri hitrostnem vodenju. Razlog je, da uporabljen dinamièen model ne uporablja dovolj natanènih parametre.

Problematika navornega krmiljenja je, da je pogojena z dobrim poznavanjem dinamiènega modela. Vsako odstopanje modela od fiziènega robota se pokaže kot napaka v sledenju ter slabšanje stabilnosti. Dodatna težava, ki se nam je pojavila je bila, da krmilnik robota PA-10 sam po sebi ne vraèa trenutnih hitrosti v sklepih in smo jih zato morali raèunati. 
Za boljše sledenje bi bilo potrebno dinamiène parametre boljše identificirati. 
%
...
Skozi delo smo avtorji predstavili dve strategiji vodenja robota ter opisali reševanje problema redundantnosti. Predstavili smo na novo narejen krmilnik, ki deluje kot strežnik ter ovrednotili njegovo delovanje. Skozi eksperimente smo pokazali zadovoljivo delovanje strežnika in opozorili na njegove pomanjkljivosti. 

Opazili smo, da vodenje robota preko vhodnih hitrostih dela prièakovano in zadovoljivo, dokler se referenca ne spreminja s preveliko hitrostjo, za kar smo tudi predstavili rešitev. Tudi admitanèno vodenje preko referenène sile se je izkazalo kot dobro s hitrim odzivom in kratkim umiritvenim èasom. Sicer je pri admitanènem vodenju bilo nekaj prenihaja, ki bi ga lahko z zmanjševanjem ojaèanj zmanjšali, vendar bi izgubili hiter odziv.
...
S poskusi smo pokazali, da je razviti hitrejši in omogoèa boljše krmiljenje v primerjavi s strežnikom, ki bi deloval z manjšo frekvenco. Integracija branja registrov senzorja sil in navorov JR3 omogoèa implementacijo admitanènega in impedanènega krmiljenja, kar s prejšnjim krmilnikom ni bilo mogoèe. 
